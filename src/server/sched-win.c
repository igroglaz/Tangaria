...
* Our timer callback
*/
static void CALLBACK timer_callback(UINT wTimerID, UINT msg, DWORD dwUser, DWORD dw1,
    DWORD dw2)
{
    timer_ticks++;
    gticks = timer_ticks;
...
static void setup_timer(void)
{
    ...
    */
    ...
    if ((timer_id = timeSetEvent(delay, resolution, timer_callback, (DWORD)0,
        TIME_PERIODIC)) == 0)
    {
...
{
    if (to_cur_free >= to_min_free) return;

    do
    {
        struct to_handler *top = (struct to_handler *)mem_alloc(sizeof(struct to_handler));
...
void install_input(void (*func)(int, int), int fd, int arg)
{
    ...
    if ((input_handlers == NULL) || (fd > biggest_fd))
    {
        if (biggest_fd < fd)
        {
            input_handlers = mem_realloc(input_handlers,
...
static void null_timer_handler(void)
{
}
...
void sched(void)
{
    ...
    while (true)
    {
        ...
        else
        {
             ...
             * the "timeout_chime" function call (which happens when a player dies).
             */
            readmask = input_mask;
            ...
            if (n < 0)
            {
                ...
                if ((timer_handler != null_timer_handler) && (errno != EINTR))
...
{
    if (timer_id != 0) stop_timer();
    timer_handler = null_timer_handler;
}
