...
static bool uncurse_object(struct player *p, struct object *obj, int strength)
{
    ...
    // low lvl players ez destroy heavely cursed items
    else if ((p->lev < 30 && curse->power > 39) || one_in_(4))
    {
        int dam = damroll(5, 5);
...
bool effect_handler_DETECT_TREASURES(effect_handler_context_t *context)
{
    ...
    if (context->cave->profile == 6 ||
        (context->cave->wpos.grid.x == -1 && context->cave->wpos.grid.y == -1)) // Maze (minotaur) dungeon
    {
...
bool effect_handler_MASS_BANISH(effect_handler_context_t *context)
{
    ...
    {
        context->radius = 2 + context->origin->player->lev / 5;
        if (context->radius > 10)
            context->radius = 10;
    }
...
bool effect_handler_NOURISH(effect_handler_context_t *context)
{
    ...
    if (streq(context->origin->player->race->name, "Ent") ||
        ...
        streq(context->origin->player->race->name, "Wraith") ||
        streq(context->origin->player->race->name, "Djinn"))
        special_race = 1;
...
bool effect_handler_TELEPORT_LEVEL(effect_handler_context_t *context)
{
    ...
    else
    {
        ...
        } else if (player_force_descend(context->origin->player, 2) || !base_depth)
        {
            up = false;
        }
