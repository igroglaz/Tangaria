    ...
    object_stack_t mode)
{
    ...
    else
    {
        ...
        {
            if (total > obj1->kind->base->max_stack) return false;
            ...
            if (mode & OSTACK_QUIVER)
            {
                ...
                else if (total > z_info->quiver_slot_size / z_info->thrown_quiver_mult)
                    return false;
            }
        }
    }
 ...
 *
 * obj1 is the first of the stacks to combine. When the stacking
 * limits (from mode1 and mode2) are the same, this stack will be larger
 * when the function returns.
 * obj2 is the second of the stacks to combine.
 * mode1 describes the behavior, most notably the upper limit on size,
 * for the first stack. Can not include OSTACK_STORE, which typically has no
 * limit on the stack size.
 * mode2 describes the behavior, most notably the upper limit on size,
 * for the second stack. Can not include OSTACK_STORE, which typically has no
 * limit on the stack size.
 */
void object_absorb_partial(struct object *obj1, struct object *obj2, object_stack_t mode1,
    object_stack_t mode2)
{
    ...
    my_assert(!(mode1 & OSTACK_STORE) && !(mode2 & OSTACK_STORE));
    ...
    if (mode1 & OSTACK_QUIVER)
    {
        ...
        else
        {
            ...
            my_assert(newsz2 < obj1->kind->base->max_stack);
        }
    }
    else if (mode2 & OSTACK_QUIVER)
    {
        /* Handle the possibly different limits. */
        int limit = z_info->quiver_slot_size / (tval_is_ammo(obj2)? 1: z_info->thrown_quiver_mult);

        newsz1 = (largest + smallest) - limit;
        newsz2 = limit;
        my_assert(newsz1 < obj1->kind->base->max_stack);
    }
    else
    {
