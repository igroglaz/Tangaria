...
static void update_birth_options(struct player *p, struct birth_options *options, bool domsg)
{
    ...
    if (OPT(p, birth_deeptown) || OPT(p, birth_zeitnot) || OPT(p, birth_ironman))
    {
        if (streq(p->clazz->name, "Half-Troll"))
            p->expfact = 130;
        else if (streq(p->clazz->name, "Human"))
            p->expfact = 100;
        else if (streq(p->clazz->name, "Half-Elf"))
            p->expfact = 115;
        else if (streq(p->clazz->name, "Elf"))
            p->expfact = 120;
        else if (streq(p->clazz->name, "Halfling"))
            p->expfact = 120;
        else if (streq(p->clazz->name, "Gnome"))
            p->expfact = 125;
        else if (streq(p->clazz->name, "Dwarf"))
            p->expfact = 130;
        else if (streq(p->clazz->name, "Half-Orc"))
            p->expfact = 120;
        else if (streq(p->clazz->name, "Dunadan"))
            p->expfact = 155;
        else if (streq(p->clazz->name, "High-Elf"))
            p->expfact = 200;
        else if (streq(p->clazz->name, "Kobold"))
            p->expfact = 115;
        else if (streq(p->clazz->name, "Yeek"))
            p->expfact = 80;
        else if (streq(p->clazz->name, "Ent"))
            p->expfact = 255;
        else if (streq(p->clazz->name, "Thunderlord"))
            p->expfact = 180;
        else if (streq(p->clazz->name, "Dragon"))
            p->expfact = 210;
        else if (streq(p->clazz->name, "Hydra"))
            p->expfact = 195;
        else if (streq(p->clazz->name, "Black-Numenor"))
            p->expfact = 140;
        else if (streq(p->clazz->name, "Damned"))
            p->expfact = 100;
        else if (streq(p->clazz->name, "Merfolk"))
            p->expfact = 125;
        else if (streq(p->clazz->name, "Barbarian"))
            p->expfact = 125;
        else if (streq(p->clazz->name, "Black-Dwarf"))
            p->expfact = 125;
        else if (streq(p->clazz->name, "Goblin"))
            p->expfact = 120;
        else if (streq(p->clazz->name, "Half-Giant"))
            p->expfact = 177;
        else if (streq(p->clazz->name, "Ogre"))
            p->expfact = 188;
        else if (streq(p->clazz->name, "Troll"))
            p->expfact = 222;
        else if (streq(p->clazz->name, "Orc"))
            p->expfact = 125;
        else if (streq(p->clazz->name, "Forest-Goblin"))
            p->expfact = 110;
        else if (streq(p->clazz->name, "Dark-Elf"))
            p->expfact = 140;
        else if (streq(p->clazz->name, "Werewolf"))
            p->expfact = 125;
        else if (streq(p->clazz->name, "Undead"))
            p->expfact = 200;
        else if (streq(p->clazz->name, "Vampire"))
            p->expfact = 235;
        else if (streq(p->clazz->name, "Maiar"))
            p->expfact = 350;
        else if (streq(p->clazz->name, "Demonic"))
            p->expfact = 250;
        else if (streq(p->clazz->name, "Balrog"))
            p->expfact = 300;
        else if (streq(p->clazz->name, "Celestial"))
            p->expfact = 225;
        else if (streq(p->clazz->name, "Nephalem"))
            p->expfact = 125;
        else if (streq(p->clazz->name, "Gargoyle"))
            p->expfact = 200;
        else if (streq(p->clazz->name, "Golem"))
            p->expfact = 135;
        else if (streq(p->clazz->name, "Pixie"))
            p->expfact = 150;
        else if (streq(p->clazz->name, "Draconian"))
            p->expfact = 290;
        else if (streq(p->clazz->name, "Titan"))
            p->expfact = 300;
        else if (streq(p->clazz->name, "Wood-Elf"))
            p->expfact = 135;
        else if (streq(p->clazz->name, "Elemental"))
            p->expfact = 175;
        else if (streq(p->clazz->name, "Frostmen"))
            p->expfact = 175;
        else if (streq(p->clazz->name, "Centaur"))
            p->expfact = 135;
