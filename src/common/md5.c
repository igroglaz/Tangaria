...
static void MD5Update(MD5_CTX *context, unsigned char *input, unsigned int inputLen)
{
    ...
    if (inputLen >= partLen)
    {
        MD5_memcpy((byte_ptr)&context->buffer[index], (byte_ptr)input, partLen);
    ...
    MD5_memcpy((byte_ptr)&context->buffer[index], (byte_ptr)&input[i], inputLen - i);
}
 ...
 */
static void MD5Final(unsigned char digest[NORMAL_WID], MD5_CTX *context)
{
    ...
    MD5_memset((byte_ptr)context, 0, sizeof(*context));
}
...
static void MD5Transform(uint32_t state[4], unsigned char block[64])
{
    ...
    /* Zeroize sensitive information. */
    MD5_memset((byte_ptr)x, 0, sizeof(x));
}
...
/* Finally we put the hashing algorithm to work */
void MD5Password(char *string)
{
    MD5_CTX context;
    ...
    for (i = 0; i < 16; i++)
    {
        *dst++ = hexval[(string[i] >> 4) & 0xf];
        *dst++ = hexval[string[i] & 0x0f];
    }
    *dst = 0;
