    ...
    switch (typ)
    {
        ...
        case STRUCT_INFO_RACE:
        {
            ...
            for (i = 0; i < max; i++)
            {
                ...
                memset(r->modifiers, 0, OBJ_MOD_MAX * sizeof(struct modifier));
        ...
        case STRUCT_INFO_CLASS:
        {
            ...
            for (i = 0; i < max; i++)
            {
                ...
                /* Transfer other fields here */
                memset(c->modifiers, 0, OBJ_MOD_MAX * sizeof(struct modifier));
                for (j = 0; j < obj_mod_max; j++)
                {
                    if ((n = Packet_scanf(&rbuf, "%hd%hd%hd%hd%b", &base, &dice, &sides, &m_bonus,
                        &lvl)) <= 0)
        ...
        case STRUCT_INFO_PROPS:
        {
            ...
            for (i = 0; i < max; i++)
            {
                ...
                if ((n = Packet_scanf(&rbuf, "%hu%hd%s%s%s", &index, &value, type, desc, name)) <= 0)
                {
                    /* Rollback the socket buffer */
                    Sockbuf_rollback(&rbuf, bytes_read);
...
static int Receive_options(void)
{
    int n;
    uint8_t ch;
    char ironman, force_descend, no_recall, no_artifacts, feelings, no_selling, start_kit,
        no_stores, no_ghost, fruit_bat, hardcore;

    if ((n = Packet_scanf(&rbuf, "%b%c%c%c%c%c%c%c%c%c%c%c", &ch, &ironman, &force_descend,
        &no_recall, &no_artifacts, &feelings, &no_selling, &start_kit, &no_stores, &no_ghost,
...
static int Receive_weather(void)
{
    int n;
    uint8_t ch;
